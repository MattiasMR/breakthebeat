...
describe("admin helpers", () => {
  it("conserva los datos de emergencia como texto y genera filtros sin tablas incompatibles", async () => {
    const workbook = buildEmergencyWorkbook([{
      ...
      contactName: "Contacto de prueba", relationship: "Familiar", contactPhone: "0012345"
    }]);
    ...
    expect(sheet.getRow(1).values).toEqual([undefined, "Código", "Nombre", "Teléfono", "Contacto emergencia", "Relación", "Teléfono emergencia"]);
    expect(sheet.getCell("D2").value).toBe("Contacto de prueba");
    expect(sheet.getCell("E2").value).toBe("Familiar");
    expect(sheet.getCell("F2").value).toBe("0012345");
    expect(sheet.autoFilter).toBe("A1:F2");
  ...
  it("genera emergencia con datos faltantes o sin participantes", async () => {
    for (const rows of [[], [{ participantCode: "TEST", displayName: "Prueba", phone: "" }]]) {
      ...
      const sheet = reopened.getWorksheet("Emergencia")!;
      expect(sheet.columnCount).toBe(6);
      expect(sheet.getCell("F1").value).toBe("Teléfono emergencia");
      expect(sheet.autoFilter).toBe(`A1:F${rows.length + 1}`);
      if (rows.length) expect(sheet.getCell("D2").value ?? "").toBe("");
    }
  });
